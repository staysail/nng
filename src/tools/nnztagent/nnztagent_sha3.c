// This file was copied from github, and then altered to remove
// the test vectors and expose single call functions.  It was under
// public domain, and remains so.
...
sha3_Update(void *priv, void const *bufIn, size_t len)
{
	...
	if (len < old_tail) { /* have no complete word or haven't started
		...
		SHA3_TRACE("because %d<%d, store it and return",
		    (unsigned) len, (unsigned) old_tail);
	...
	if (old_tail) { /* will have one word to process */
		SHA3_TRACE(
		    "completing one word with %d bytes", (unsigned) old_tail);
		/* endian-independent code follows: */
		len -= old_tail;
		while (old_tail--)
			ctx->saved |= (uint64_t)(*(buf++))
	...
	SHA3_TRACE(
	    "have %d bytes left to process, save them", (unsigned) tail);
 ...
 * M || 01, we would simply use 1 to start padding. */
	...
	ctx->s[ctx->wordIndex] ^= (ctx->saved ^
	    ((uint64_t)(
	        (uint64_t)(0x02 | (1 << 2)) << ((ctx->byteIndex) * 8))));
#else
...
// Output must hold 256 bytes.
void
nnzt_agent_sha3_256(const char *src, size_t len, char *output)
{
	const void * hash;
	sha3_context ctx;
	...
	memcpy(output, hash, 256 / sizeof(uint8_t));
}