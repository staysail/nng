...
A proxy server sits in front.  It sets up associations with
nng instances using a shared API over some kind of IPC.  (This could
itself even be a nanomsg based service.)

Messages we would need:

BIND(NWID, local port) - (port 0 means choose a random one). gives back own identity.
...
SEND(remote addr) - sends remote addr
...
RECV() - get remote addr with packet
